...
class STS3032Actuator(VoltageControlledActuator):
    """
    Feetech STS3032 6.0v
    ...
    def __init__(self, testbench_class: Testbench):
        super().__init__(
            ...
            kp=32,
            # This gain, if multiplied by a position error and firmware KP, gives duty cycle
            # It was determined using an oscilloscope and STS3032 actuators
            # here, firmware_kp = kp
            error_gain=0.166,
            # self.error_gain = 0.001 * np.rad2deg(1.0)
            # Maximum allowable duty cycle, also determined with oscilloscope
        ...
        # self.default_max_velocity = (3400 * 2 * np.pi) / 4096
        # STS3032 spec: 0.09 sec / 60 deg @ 6V
        # 60 deg = pi/3 rad
        self.default_max_velocity = (np.pi / 3.0) / 0.09
    ...
    def initialize(self):
        # Torque constant [Nm/A] or [V/(rad/s)]
        # self.model.kt = Parameter(0.784532, 0.05, 2.5)  # docs says 8 kg.cm / A
        # STS3032: 4.5 kg.cm peak stall torque, 1.2 A stall current
        # 4.5 kg.cm ~= 0.441 Nm, 0.441 / 1.2 ~= 0.37 Nm/A
        self.model.kt = Parameter(0.37, 0.02, 1.5)

        self.model.error_gain_ratio = Parameter(1.0, 0.1, 10.0)

        # Motor resistance [Ohm]
        # self.model.R = Parameter(2.0, 0.1, 10.0)
        # Rough estimate from 6V / 1.2A = 5 Ohm
